import type { DictionaryKey } from "./es";

export const en: Record<DictionaryKey, string> = {
  // Nav & section labels
  "nav.about":      "About",
  "nav.experience": "Experience",
  "nav.projects":   "Projects",
  "nav.skills":     "Skills",
  "nav.contact":    "Contact",

  // Hero / mobile header
  "header.role":    "FULLSTACK DEV",
  "skip.toContent": "Skip to content",

  // About section paragraphs
  "about.p1": "Fullstack developer focused on backend and DevOps. I design normalized databases, build scalable RESTful APIs, and automate processes that solve real problems.",
  "about.p2": "I've deployed applications on my own VPS with custom domain, SSL, and Docker Compose. I take projects end to end — from data modeling to the frontend with React.",
  "about.p3": "Graduated in University Technician in Programming from UTN La Rioja.",
  "about.downloadCV":         "Download CV",
  "about.stat.experience":    "yrs exp.",
  "about.stat.projects":      "projects",
  "about.stat.clients":       "real clients",

  // Timeline
  "timeline.tab.work":      "Work",
  "timeline.tab.education": "Education",

  // Skills group labels
  "skills.group.backend":   "Backend",
  "skills.group.databases": "Databases",
  "skills.group.devops":    "DevOps & Infrastructure",
  "skills.group.tools":     "Tools",
  "skills.group.frontend":  "Frontend",

  // Contact
  "contact.text":     "Available for freelance work, full-time projects, or collaborations. If you have something in mind, reach out directly.",
  "contact.location": "Pituil, La Rioja, Argentina · Available remote",

  // Footer
  "footer.credit": "Designed and built by",

  // Project detail page
  "project.back":                 "← Projects",
  "project.viewSource":           "View source",
  "project.viewLive":             "View demo",
  "project.status.production":    "In production",
  "project.status.wip":           "In development",
  "project.status.mvp":           "MVP",
  "project.status.archived":      "Archived",
  "project.section.problem":      "The Problem",
  "project.section.architecture": "Architecture",
  "project.section.stack":        "Why this Stack",
  "project.section.challenges":   "Technical Challenges",
  "project.section.learnings":    "Key Learnings",
  "project.video":                "Video demo",
  "project.screenshots":          "Screenshots",
  "project.noScreenshots":        "Screenshots in progress",
  "project.notFound":             "Project not found",
  "project.next":                 "Next project",

  // 404
  "notFound.title":   "404",
  "notFound.message": "Page not found",
  "notFound.back":    "Go back home",
};
